# 銀行、詐騙與保險 Banking, Fraud & Insurance

## Reference Fit

Use this file when the user need involves:
- Bank accounts/cards, remittance, mobile payments, credit report, brokerage/TDCC, insurance policy/claim, FSC/financial dispute.
- Fraud transfer, suspicious call/debt collection, lost cards, bank-account recovery, invoice-prize payout account.
- Personal financial records that touch BLI/labor insurance only as account/record lookup, not labor-rights enforcement.
...
- The question is primarily tax filing, telecom OTP/phone recovery, or labor dispute/benefit eligibility.

Adjacent references:
- `references/emergency-safety.md` for active fraud crisis, coercion, threats, or immediate police/bank action.
- `references/taxation.md` for income-tax, withholding, e-invoice tax rules, or tax payment details.
- `references/telecom-digital-life.md` for SIM/OTP/account lockout that blocks bank access.

Fast disambiguators:
- Ask whether money/card access is at risk now; if yes, bank/card issuer and 165 come before background explanation.
- Ask whether the issue is with a bank, insurer, broker, remittance service, telecom OTP, or government payment portal.
