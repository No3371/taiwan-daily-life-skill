# 電信與數位生活 Telecom & Digital Life

## Reference Fit

Use this file when the user need involves:
- SIM/eSIM, mobile plan, telecom/cable/broadband dispute, carrier identity verification, mobile number/OTP, phone loss.
- Government digital identity blockers: MyData, natural-person certificate, TW FidO, ARC/ID login format, portal outage/maintenance.
- Convenience-store digital tasks only when the issue is printing, bill payment, ticketing, or kiosk flow.
...
- The question is primarily fraud transfer, banking recovery, identity-document issuance, or parcel/consumer dispute.

Adjacent references:
- `references/banking-fraud-insurance.md` for bank/card account lockout, fraud transfer, financial dispute, or credit issue.
- `references/government-identity.md` for ARC/APRC, natural-person certificate issuance, MyData document source, or official ID record.
- `references/mail-parcels-consumer-life.md` for delivery-platform, e-commerce, or consumer-service dispute.

Fast disambiguators:
- Ask exact ID type/format, carrier/account name, phone access, OTP access, browser/app, and error text.
- Ask whether the service failure is user-account-specific, carrier-specific, or a public portal outage.
