# 天氣、地震、放假與服務異動 Weather, Earthquakes, Holidays & Closures

## Reference Fit

Use this file when the user need involves:
- Weather forecast, warnings, earthquake reports/intensity, typhoon stop-work/school, DGPA office calendar, make-up workdays.
- Current closure/status checks: school, bank, post office, clinic, transport, road, public-service system outage.
- Holiday/labor-day distinctions where sector and city/county determine the answer.
...
- The question is primarily transport booking/admin, environmental complaint, or civil-defense alert/shelter.

Adjacent references:
- `references/transportation-vehicles.md` for booking, fares, fines, vehicle admin, lost items, or operator-specific transport process.
- `references/environment-public-health.md` for AQI, sanitation, pests, water quality, pollution/noise, or beach/mountain/river safety.
- `references/civil-defense-alerts-wartime.md` for air-raid alerts, Wan-An drill, shelter lookup, or national phone alerts.

Fast disambiguators:
- Ask city/county, sector (school/work/bank/post/clinic/transport), exact date, and whether this is planned holiday or current disaster status.
- Ask whether the user needs official closure, operator service status, weather/earthquake facts, or safety action.
